...
export default function AdminOrdersPage() {
  ...
  const changeStatus = async (orderId: string, newStatus: OrderStatus) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === orderId ? (data.order as AdminOrderDetail) : o))
      );
  ...
  return (
    <main className="relative mx-auto max-w-7xl px-6 py-8 md:py-12 lg:px-8">
      <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        >
          ...
          <select
            ...
            className="h-11 rounded-full border border-dc-ink/10 bg-white/80 px-5 text-[10px] font-bold uppercase tracking-[0.2em] text-dc-ink/80 shadow-sm outline-none transition-all focus:border-dc-caramel/50 focus:bg-white focus:shadow focus:ring-4 focus:ring-dc-caramel/10 dark:border-white/10 dark:bg-white/5 dark:text-white/80"
          >
      ...
      {!loading && visibleOrders.length === 0 && !error && (
        ...
        >
          <p className="text-base text-dc-ink/60 dark:text-white/60">
      ...
      {!loading && visibleOrders.length > 0 && (
        ...
        >
          <div className="grid grid-cols-[1.6fr_1.1fr_1fr_1fr_1.4fr] gap-4 border-b border-dc-ink/8 bg-dc-sand/10 px-6 py-4 text-[10px] font-bold uppercase tracking-[0.2em] text-dc-ink/50 dark:border-white/10 dark:bg-white/5 dark:text-white/50">
            <span>Order</span>
            <span>Date</span>
            <span>Status</span>
            <span>Total</span>
          ...
          >
            {visibleOrders.map((order) => {
              ...
              return (
                <motion.div
                  key={order.id}
                  className="border-b border-dc-ink/5 px-6 py-6 text-sm text-dc-ink dark:border-white/5 dark:text-white"
                >
                  <div className="grid grid-cols-[1.6fr_1.1fr_1fr_1fr_1.4fr] gap-4">
                    <div className="flex flex-col">
                      ...
                      <span className="mt-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-dc-ink/40 dark:text-white/40">
                    ...
                    <span className="text-base font-bold text-dc-ink dark:text-white">
                      {total}{" "}
                    ...
                    <div className="flex flex-col">
                      <span className="text-xs font-semibold text-dc-ink dark:text-white">
                        {shippingName}
                      </span>
                      <span className="mt-1.5 text-[10px] font-medium text-dc-ink/50 dark:text-white/50">
                        {order.customer_email ?? "No email"}
                      </span>
                    </div>
                  </div>

                  <div className="mt-5 flex flex-col gap-3 border-t border-dc-ink/5 pt-5 dark:border-white/5 md:flex-row md:items-center md:justify-between">
                    <motion.button
